...
const flagPoll = {x_pos: 1500, y_pos: 240, width: 5, height: 200, isReached: false};
...
function setup() {
	const MOUNTAIN_DENSITY = 1500;
	const MAX_MOUNTAIN_COUNT = 5;
	let mountainCount = 0;

	const CACTUS_DENSITY = 300;
	const MAX_CACTUS_COUNT = 1000;
	let cactusCount = 0;

	const TREE_DENSITY = 400;
	const MAX_TREE_COUNT = 20;
	let treeCount = 0;

	const CLOUD_DENSITY = 5000;
...
function stageEnd() {
	noLoop();
	redraw();
	text("Stage Complete. Press Enter to Continue", 100, 100);
}
...
function checkFlagpole() {
	if(abs(dustDevil.x_pos - flagPoll.x_pos) < 15) {
...
function drawScoreBoard() {
	fill(0, 0, 0)
	textSize(50)
	text(collectedCacti, 45, 50)
	for(let i = 0; i < remainingLives; i++) {
		fill(0, 0, 0)
		ellipse((i+1) * 30, 70, 20, 20)
	}
...
function drawFlagPoll() {
	push();
	fill(135, 135, 135);
	rect(flagPoll.x_pos, 240, 5, 200);
	//flag
	fill(255, 0, 0);
	rect(flagPoll.x_pos, 240, 50, 30);
	pop();
}
